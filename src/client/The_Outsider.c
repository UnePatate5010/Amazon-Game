...
int score(struct game_state_t *state) {
    ...
    int passive_queens_us = 0;
    int passive_queens_ennemy = 0;
    ...
    for (unsigned int player=0; player<NUM_PLAYERS; player++) {
        for (unsigned int queens_index = 0; queens_index < state->num_queens; queens_index++) // For all queens.
        {
            ...
            if(all_moves2.dsts_i[i] == all_moves2.dsts_i[i + 1]) 
            {
                if(player == state->player_id)
                {
                    passive_queens_us++;
                }
                if(player == 1-state->player_id)
                {
                    passive_queens_ennemy++;
                }
            }
            ...
            for (unsigned int k = all_moves2.dsts_i[i]; k < all_moves2.dsts_i[i + 1]; k++)
            {
                ...
                score_queen += all_moves2.dsts_i[j + 1] - all_moves2.dsts_i[j] + 1;
            }
            counts[player] += score_queen + score_queen * nb_dir_pos(state->graph, state->queens, state->num_queens, all_moves2.dsts_i[i])/10;
        }
    }
    return counts[state->player_id] + (passive_queens_ennemy - passive_queens_us)*counts[state->player_id]/state->num_queens - counts[1-state->player_id] - (passive_queens_us - passive_queens_ennemy)*counts[1-state->player_id]/state->num_queens;
}
