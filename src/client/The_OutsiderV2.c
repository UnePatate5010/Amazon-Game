...
int score(struct game_state_t *state) {
    ...
    for (unsigned int player=0; player<NUM_PLAYERS; player++) {
        for (unsigned int queens_index = 0; queens_index < state->num_queens; queens_index++) // For all queens.
        {
            ...
            left_dirs[player] += nb_dir_pos(state->graph, state->queens, state->num_queens, i);
    ...
    return counts[state->player_id] - counts[1-state->player_id] + 10*(left_dirs[state->player_id] - left_dirs[1-state->player_id]);
}
